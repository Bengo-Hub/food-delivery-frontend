...
/**
 * Post-login outlet selection for ordering-frontend.
 *
 * Fetches the tenant's outlet list from auth-api (not ordering-backend).
 * - Single outlet: auto-selects without showing UI.
 * - Multiple outlets: shows a card selector with last-used outlet highlighted.
 * - Stores selection in localStorage ('ordering-selected-outlet-id') and syncs
 *   to X-Outlet-ID header via outletIdGetter in base.ts.
 */
...
export const ORDERING_SELECTED_OUTLET_KEY = 'ordering-selected-outlet-id';
...
function getLastSelectedOutletId(): string | null {
  ...
  try { return localStorage.getItem(ORDERING_SELECTED_OUTLET_KEY); } catch { return null; }
}
...
function SelectOutletContent() {
  ...
  const tenantSlug = (user as any)?.tenant_slug || (user as any)?.tenantSlug || '';
  ...
  const handleSelect = useCallback(
    (outlet: OutletItem) => {
      ...
      const outletOption: OutletOption = {
        ...
        code: outlet.code,
        name: outlet.name,
        ...(outlet.use_case !== undefined && { useCase: outlet.use_case }),
        ...(outlet.is_hq !== undefined && { isHq: outlet.is_hq }),
      };
      selectOutlet(outletOption);
      localStorage.setItem(ORDERING_SELECTED_OUTLET_KEY, outlet.id);
      redirect(returnTo);
    },
  ...
  if (error) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-background p-4">
        <div className="w-full max-w-sm rounded-xl border border-destructive/20 bg-destructive/5 p-6 text-center">
    ...
    <div className="flex min-h-screen flex-col items-center justify-center bg-background px-4 py-8">
      <div className="w-full max-w-lg space-y-6">
        <div className="text-center">
          ...
          <h1 className="text-2xl font-bold text-foreground">Select Outlet</h1>
        ...
        <div className="space-y-3">
          {orderedOutlets.map((outlet) => {
            ...
            const useCaseColor = USE_CASE_COLORS[useCase] ?? 'bg-muted text-muted-foreground';
              ...
              >
                <div className="flex items-center gap-4">
                  <div className={[
                    ...
                    isLastUsed ? 'bg-primary/20' : 'bg-muted group-hover:bg-primary/20',
                  ].join(' ')}>
                    <MapPin className={[
                      'h-5 w-5 transition-colors',
                      isLastUsed ? 'text-primary' : 'text-muted-foreground group-hover:text-primary',
                    ].join(' ')} />
                  </div>
                  <div>
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      {outlet.is_hq && (
                        <span className="flex items-center gap-1 rounded-full bg-yellow-500/20 px-2 py-0.5 text-xs font-medium text-yellow-600 dark:text-yellow-300">
                ...
                <div className="shrink-0 pl-2">
                  {isSelecting ? (
                    ...
                    <CheckCircle2 className="h-5 w-5 text-muted-foreground/40 transition-colors group-hover:text-primary" />
                  )}
                </div>
              </button>
            );

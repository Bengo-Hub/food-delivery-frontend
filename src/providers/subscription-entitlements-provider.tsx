...
/**
 * Bridges ordering-frontend's useSubscription hook + the platform feature catalog into the shared
  ...
  const catalog = useMemo<Record<string, FeatureCatalogEntry>>(() => {
    ...
    for (const f of catalogData?.features ?? []) {
      ...
      map[f.featureCode] = {
        ...
        label: f.label ?? "",
        // exactOptionalPropertyTypes: only set the optional numeric field when present.
        ...(f.minTierOrder !== undefined ? { minTierOrder: f.minTierOrder } : {}),
      };
    }
    return map;
  }, [catalogData]);
